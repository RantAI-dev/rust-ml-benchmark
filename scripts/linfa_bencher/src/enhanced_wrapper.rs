...
fn run_single_benchmark(dataset: &str) -> Result<Vec<SingleBenchResult>, Box<dyn Error>> {
    let temp_output = format!("/tmp/linfa_single_{}.json", std::process::id());
    
    let status = Command::new("cargo")
        .args(&["run", "--release", "--bin", "linfa_bencher", "--", 
                "--dataset", dataset, "--output", &temp_output])
        .status()?;
    
    if !status.success() {
        return Err("Linfa benchmark failed".into());
    }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("🚀 Enhanced Linfa Benchmark Suite");
    ...
    let dataset_info = DatasetInfo {
        ...
        preprocessing_notes: "Linear scaling, categorical encoding".to_string(),
    };
    ...
    // Write header
    writeln!(file, "# Enhanced Linfa Benchmark Results")?;
