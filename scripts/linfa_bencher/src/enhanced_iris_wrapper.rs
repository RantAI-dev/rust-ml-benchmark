...
fn collect_system_info() -> SystemInfo {
    ...
    SystemInfo {
        ...
        os_version,
        rust_version,
        linfa_version: "0.7.0".to_string(),
        timestamp: Local::now(),
    }
}

fn run_single_iris_benchmark(dataset: &str) -> Result<Vec<SingleBenchResult>, Box<dyn Error>> {
    let temp_output = format!("/tmp/linfa_iris_single_{}.json", std::process::id());
    
    // Convert relative path to absolute if needed
    let dataset_path = if dataset.starts_with('/') {
        dataset.to_string()
    } else {
        // If it's a relative path, make it relative to the root project directory
    ...
    let status = Command::new("cargo")
        .args(&["run", "--release", "-p", "linfa_bencher", "--bin", "iris_bencher", "--", 
                "--dataset", &dataset_path, "--output", &temp_output])
        .current_dir("/home/shirologic/Desktop/RantAI/repo/rust-ml-benchmark/scripts")
        .status()?;
    
    if !status.success() {
        return Err("Linfa iris benchmark failed".into());
    }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("🚀 Enhanced Linfa Iris Benchmark Suite");
    ...
    let dataset_info = DatasetInfo {
        ...
        preprocessing_notes: "Linear scaling, binary SVM (Setosa vs Others)".to_string(),
    };
    ...
    for i in 0..args.warmup {
        ...
        let _ = run_single_iris_benchmark(&args.dataset)?;
        println!("✓");
    }
    ...
    for i in 0..args.runs {
        ...
        let run_results = run_single_iris_benchmark(&args.dataset)?;
    ...
    // Write header
    writeln!(file, "# Enhanced Linfa Iris Benchmark Results")?;
    ...
    println!("\n✅ Enhanced iris benchmark complete! Results saved to: {}", args.output);
